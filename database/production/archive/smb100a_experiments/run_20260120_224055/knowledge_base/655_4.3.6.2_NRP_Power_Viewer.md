---
chapter_index: 655
title: "4.3.6.2_NRP_Power_Viewer"
--- 

# 4.3.6.2_NRP_Power_Viewer
...
connect the sensor directly at a [USB] interface. Requires the following cables, depending on the used sensor type:
...
connection the sensor indirectly via USB hub with external power supply unit Requires the following cables, depending on the used sensor type:

-R&S NRP-ZKU (USB interface cable) for R&S NRPxx power sensors

-R&S NRP-Z3 or R&S NRP-Z4 (USB adapter cables) for sensors of the R&S NRP-Zxx family

connect an R&S NRPxxN power sensors via LAN Using the Ethernet interface requires PoE (Power over Ethernet) to provide the electrical power.
