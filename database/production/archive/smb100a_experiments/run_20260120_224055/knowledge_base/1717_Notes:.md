---
chapter_index: 1717
title: "Notes:"
--- 

# Notes:

Notes:

If specified, a level offset [:SOURce<hw>]:POWer[:LEVel][:IMMediate]: OFFSet is included according to the formula:

Minimum level + OFFSet

In addition to numerical values, you can increase or decrease the values step by step with the UP and DOWN according to the step width defined with [:SOURce<hw>]: POWer:STEP[:INCRement] .

Operating Manual 1407.0806.32 ─ 23

384

R&S ® SMB100A

Remote Control Commands

SOURce Subsystem

The RF output is activated with :OUTPut<hw>[:STATe] on page 317 (RF ON / RF OFF).

